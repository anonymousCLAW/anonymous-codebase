...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    PRETRAINED_MODEL="pretrain_sri_hidden_identity_${VIEWS_TYPE}"
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        FINETUNED_MODEL="finetuned_sri_transformer_${VIEWS_TYPE}_online_${TRANSFORM_NAME}_decoder-only-${DECODER_ONLY}-epochs-1-${LAMB}"
        ...
        EPOCHS=1 \
        CHECKPOINT="${PRETRAINED_MODEL}/ckpt_pretrain_ep0199_step0310000.pth" \
        ...
        ./experiments/adv_train_transformer.sh 0 2 1 false 1 1 true 1 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.25 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 1 $LAMB | tee adv_pretrain_adv_train_${LAMB}.txt
        ...
        ./experiments/attack_and_test_transformer.sh 0 2 1 false 1 1 true 1 false false false v2-3-z_rand_1-pgd_3_no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-${VIEWS_TYPE}-$TRANSFORM_NAME_FINE-full-dec_only-${DECODER_ONLY}-AT-${LAMB} $TRANSFORM_NAME 0.5 0.5 0.01 $DATASET_NAME 1 $ADVERSARIAL_MODEL $NUM_REPLACE 0 true true false "Best_F1"
