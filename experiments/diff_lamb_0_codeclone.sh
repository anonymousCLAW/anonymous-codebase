...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        EXPT_NAME="v2-3-z_rand_1-pgd_3-no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-$TRANSFORM_NAME_FINE-$FINETUNED_MODEL-AT-lamb-${LAMB}-fre-2"
        ./experiments/adv_train_seq2seq_codeclone.sh 1 2 1 false 1 1 true 1 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 2 $LAMB | tee adv_pretrain_adv_train_${LAMB}_2.txt
        ...
        ./experiments/adv_train_seq2seq_codeclone.sh 1 2 1 false 1 1 true 1 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 5 $LAMB | tee adv_pretrain_adv_train_${LAMB}5.txt
