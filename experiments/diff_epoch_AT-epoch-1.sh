...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        FINETUNED_MODEL="finetuned_c2s_hidden_identity_${VIEWS_TYPE}_online_${TRANSFORM_NAME}_decoder-only-${DECODER_ONLY}-epochs-1"
        ...
        #./experiments/adv_train_seq2seq.sh 1 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 2 0.4
        ...
        ./experiments/adv_train_seq2seq.sh 1 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 5 0.4
        #attack and test the trained model
        # EXPT_NAME="v2-3-z_rand_2-pgd_3-no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-$TRANSFORM_NAME_FINE-$FINETUNED_MODEL-AT-FRE-10"
        #./experiments/adv_train_seq2seq.sh 1 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 10 0.4
        # attack and test the trained model
        EXPT_NAME="v2-3-z_rand_1-pgd_3-no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-$TRANSFORM_NAME_FINE-$FINETUNED_MODEL-AT-FRE-5"
