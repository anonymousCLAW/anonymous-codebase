...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        FINETUNED_MODEL="finetuned_sri_${VIEWS_TYPE}_online_${TRANSFORM_NAME}_decoder-only-${DECODER_ONLY}-epochs-1-${LAMB}"
        ...
        EPOCHS=1 \
        CHECKPOINT="${PRETRAINED_MODEL}/ckpt_pretrain_ep0086_step0025000.pth" \
        MODEL_NAME=${FINETUNED_MODEL} \
            #time make finetune-contracode-code-completion

        # adversarial training starting from the partially finetuned model from above
        NUM_REPLACE=1500
        DATASET_NAME_SMALL="py150"
        TRANSFORM_NAME="transforms.Replace"
        TRANSFORM_NAME_FINE="transforms.Replace"
        EXPT_NAME="v2-3-z_rand_1-pgd_3-no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-$TRANSFORM_NAME_FINE-$FINETUNED_MODEL-AT-lamb-${LAMB}"
        ./experiments/adv_train_seq2seq-code-completion.sh 0 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 1 $LAMB | tee adv_pretrain_adv_train_${LAMB}.txt
