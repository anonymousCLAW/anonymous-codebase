...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        ./experiments/adv_train_seq2seq.sh 0 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 1 ${LAMB} | tee adv_pretrain_adv_train_${LAMB}.txt
        ...
        ./experiments/attack_and_test_seq2seq.sh 0 2 1 false 1 1 true 3 false false false v2-3-z_rand_1-pgd_3_no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-${VIEWS_TYPE}-$TRANSFORM_NAME_FINE-full-dec_only-${DECODER_ONLY}-AT-${LAMB} $TRANSFORM_NAME 0.5 0.5 0.01 $DATASET_NAME 1 $ADVERSARIAL_MODEL $NUM_REPLACE 0 true true false "Latest"
