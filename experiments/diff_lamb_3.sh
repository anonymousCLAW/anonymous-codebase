...
VIEWS=("adv-contra_transforms.Combined_1_2")
...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        FINETUNED_MODEL="finetuned_c2s_hidden_identity_${VIEWS_TYPE}_online_${TRANSFORM_NAME}_decoder-only-${DECODER_ONLY}-epochs-1-${LAMB}"
        ...
        EXPT_NAME="v2-3-z_rand_1-pgd_3-no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-$TRANSFORM_NAME_FINE-$FINETUNED_MODEL-AT-lamb-${LAMB}"
        ./experiments/adv_train_seq2seq.sh 1 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 1 ${LAMB} | tee adv_pretrain_adv_train_${LAMB}.txt
        ...
        ./experiments/attack_and_test_seq2seq.sh 1 2 1 false 1 1 true 3 false false false v2-3-z_rand_1-pgd_3_no-$TRANSFORM_NAME-$DATASET_NAME_SMALL-${VIEWS_TYPE}-$TRANSFORM_NAME_FINE-full-dec_only-${DECODER_ONLY}-AT-${LAMB} $TRANSFORM_NAME 0.5 0.5 0.01 $DATASET_NAME 1 $ADVERSARIAL_MODEL $NUM_REPLACE 0 true true false "Latest"
