...
for VIEWS_TYPE in "${VIEWS[@]}"; do
    ...
    for DECODER_ONLY in "${TRAIN_DECODER_ONLY[@]}"; do
        ...
        FINETUNED_MODEL="finetuned_sri_${VIEWS_TYPE}_online_${TRANSFORM_NAME}_decoder-only-${DECODER_ONLY}-epochs-10-${LAMB}"
        ...
        #./experiments/adv_train_seq2seq-code-completion.sh 1 2 1 false 1 1 true 3 false false false $EXPT_NAME $TRANSFORM_NAME_FINE 0.5 0.5 0.01 $DATASET_NAME 1 $FINETUNED_MODEL $NUM_REPLACE 0 $DECODER_ONLY 1 $LAMB | tee adv_pretrain_adv_train_1.txt
